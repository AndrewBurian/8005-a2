...
Prototype:	int select_server(int port)
...
Description:
  A simple select-centered echo server
  but cheating and using level-triggered epoll instead of select.
  same thing.
...
int select_server(int port){
  ...
  // number of active epoll descriptors
  int fdCount = 0;
  ...
  // the size remaining in the buffer
  int buflen = 0;
  ...
  addEvent.events = EPOLLIN | EPOLLERR | EPOLLHUP;
